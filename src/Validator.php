...
use validator\Contracts\ErrorBagInterface;
use validator\Rules\AlNum;
use validator\Rules\Alpha;
use validator\Rules\ArrayRule;
use validator\Rules\Boolean;
use validator\Rules\Contracts\RuleInterface;
use validator\Rules\Email;
use validator\Rules\File;
use validator\Rules\IP;
use validator\Rules\Length;
use validator\Rules\Max;
use validator\Rules\Min;
use validator\Rules\Number;
use validator\Rules\Password;
use validator\Rules\Required;
use validator\Rules\Same;
...
class Validator
{
        ...
        foreach ($data as $field => $value) {
            ...
            foreach ($userRules as $rule) {
                ...
                if (!isset($this->rules[ $ruleName ])) {
                    trigger_error(sprintf("Rule `%s` is not recognized, please register it using \validator\Validator::addRule(\$ruleName, \$ruleClass) method",
                        $ruleName));
                }
    ...
    public function addRule($id, $rule)
    {
        if (!is_callable($rule) && !$rule instanceof RuleInterface) {
            throw new \Exception("Rule must be callable or an instance of " . '\validator\Rules\Contracts\RuleInterface');
        }
